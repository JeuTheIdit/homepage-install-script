...
## Repository overview
This is a bash script that installs and updates Homepage from source.

### Variables
- `APP`: Homepage
- `LOCAL_IP`: Local IPv4 address of machine
- `RELEASE`: Latest release version, pullsed from Github.
...
- Checks the presence of the version file. If it does not exist, proceed as a new installation. If it exists and does not match current release, proceed as update. If it exists and matches current release, exit.
- Updates packages, upgrades packages, and updates pnpm (or installs if not present).
- Downloads the source code using `RELEASE` tag and places it in `/tmp`.
...
- Does the following if it is a new installation, otherwise skip for update only.
  - Copy `/opt/${APP}/src/skeleton/*` to teh config directory `/opt/${APP}/config/`. The files in `/src/skeleton/*` are default configuration files. Do not copy these to the config directory if there is an existing install, as it is assumed that configuration files are already present.
  - Create and populate environment file `.env` with allowed hosts. Do not create this if there is an existing install, as it is assumed to already be created.
